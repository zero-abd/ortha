 ...
 */
export function runLiveTurn(text: string, deps: LiveDeps, apiBase: string, images?: readonly string[], deepResearch?: boolean): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    ...
    ws.addEventListener("open", () => {
      ...
      ws.send(JSON.stringify({ type: "user_message", text, ...(images && images.length > 0 ? { images } : {}), ...(deepResearch ? { deepResearch: true } : {}) }));
    });
